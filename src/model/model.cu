...
Status LoadModel(const ModelConfig& cfg, Model* m, cudaStream_t stream) {
  ...
  for (auto& l : m->layers) {
    ws = std::max(
        ws, DecoderLayerWorkspaceBytes(
                cfg.max_prefill, l.is_full_attention, l.has_ple, cfg.hs, cfg.E,
                cfg.moe_is, cfg.shared_is, cfg.topk, &l.full,
                std::max(l.attn_hc.lowrank, l.mlp_hc.lowrank)));
  }
  m->ws_bytes = ws;
