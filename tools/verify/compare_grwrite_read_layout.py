"""Check fused boundary lifetime and unchanged layout after full HTTP."""
import argparse
import hashlib
import json
...
def main():
    ...
    helper_start = old.index('size_t AttnWs(')
    helper = old[helper_start:old.index('\n}  // namespace', helper_start)]
    old_header = subprocess.check_output(
        ['git', 'show', baseline_ref + ':include/q4t/model/decoder_workspace.h'],
        text=True, cwd=ROOT)
    struct_start = old_header.index('struct DecoderWorkspaceLayout')
    old_struct = old_header[struct_start:old_header.index('\n};', struct_start)+3]
    builder_start = old.index('DecoderWorkspaceLayout MakeDecoderWorkspaceLayout(')
    old_builder = old[builder_start:old.index('std::array<DecoderResourceView', builder_start)]
    old_layout = old_struct + '\n' + old_builder
    # Forward order intentionally changes: the new boundary produces normed
    # during attention Write. Numerical checks independently cover that change.
