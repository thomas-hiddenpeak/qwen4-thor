"""Compare paired GRRead gate/mix arithmetic after full HTTP acceptance."""
import argparse
import hashlib
import json
...
def main():
    ...
    original = subprocess.check_output(
        ...
        cwd=ROOT, text=True)
    candidate = (ROOT / 'src/model/hyperconnection.cu').read_text()
    def extract(source):
        helpers = source[source.index('__device__ __forceinline__ float Bf16ToFloat'):
                         source.index('// GroupedGemmaRMSNorm:')]
        kernels = source[source.index('__global__ void MixGateKernel'):
                         source.index('// Precompute the inject gate')]
        return helpers + kernels
    template = Path(__file__).with_suffix('.cu.in')
    generated = template.read_text().replace('@ORIGINAL@', extract(original)).replace(
