"""Check normed/MoE alias boundaries and compiled capacity after full HTTP."""
import argparse
import hashlib
import json
...
int main() {
    ...
    for (int max_len : {8192, 208896, 262144}) {
      ...
      for (bool full : {false, true}) {
        for (bool ple : {false, true}) {
          ...
          Require(now.normed == now.moe, "alias offset");
          Require(now.moe_bytes >= hyper, "model MoE capacity contains normed");
          Require(old.total_bytes == now.total_bytes + hyper, "capacity delta");
          Require(old.attention == now.attention && old.moe == now.moe &&
                  old.moe_gemm == now.moe_gemm && old.hc_gemm == now.hc_gemm &&
                  old.ple == now.ple && old.mixed == now.mixed &&
                  old.block == now.block, "unchanged prefix offsets");
          Require(old.combined == now.combined + hyper &&
                  old.ple_trunk == now.ple_trunk + hyper, "shifted tail offsets");
          Require(old.attention_bytes == now.attention_bytes &&
                  old.moe_bytes == now.moe_bytes && old.ple_bytes == now.ple_bytes,
                  "unchanged submodule requirements");
          // normed is deliberately excluded from the disjoint physical list;
          // it is the only view allowed to overlap the MoE region.
          const std::array<std::array<size_t, 2>, 9> regions = {{
              {now.attention, now.attention_bytes},
              {now.moe, std::max(now.moe_bytes, hyper)},
              {now.moe_gemm, baseline::kGemmWs},
              {now.hc_gemm, baseline::kGemmWs}, {now.ple, now.ple_bytes},
              {now.mixed, hidden}, {now.block, hidden}, {now.combined, hyper},
              {now.ple_trunk, ple ? hyper : 0}}};
          for (size_t i = 0; i < regions.size(); ++i) {
            const auto a = regions[i];
            Require(a[0] % 256 == 0 && a[0] <= now.total_bytes &&
                    a[1] <= now.total_bytes - a[0], "alignment and bounds");
            for (size_t j = i + 1; j < regions.size(); ++j) {
              const auto b = regions[j];
              Require(!a[1] || !b[1] || a[0] + a[1] <= b[0] ||
                      b[0] + b[1] <= a[0], "physical regions disjoint");
            }
          }
          for (bool known_full : {false, true}) {
            ...
            const auto expected = candidate::MakeDecoderWorkspaceLayout(
                t, full, ple, 4, 2560, 512, 640, 640, 10, w);
            Require(expected.total_bytes == DecoderLayerWorkspaceBytes(
                t, full, ple, 2560, 512, 640, 640, 10, w), "linked capacity");
          }
          ++cases;
        }
      }
  ...
  std::printf("PASS: %d layouts; %d linked capacities; only normed/MoE alias\n",
              cases, cases * 2);
