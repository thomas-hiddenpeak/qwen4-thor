"""Check chunk sequence API against the legacy model call chain after full HTTP."""
import argparse
import hashlib
import json
...
def main():
    ...
    command = [compiler, '-std=c++23', '--expt-relaxed-constexpr', '-O3',
               ...
               '-Xlinker=--wrap=cudaMemcpyAsync', '-Xlinker=--start-group']
    ...
    with (out / 'result.log').open('w') as log:
        result = subprocess.run([str(out / 'comparison'), str(args.model_dir)], stdout=log,
