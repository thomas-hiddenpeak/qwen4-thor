...
s=root/'tools/verify/moe_scale_hc_read_l2';p=root/'.q4t-work/prepared/moe-scale-hc-read-l2-20260924';old=root/'.q4t-work/e2e/moe-scale-hc-mlp-read-l1-20260924'
def sha(f):
 h=hashlib.sha256()
 with f.open('rb') as z:
...
for name in ['norm.cu','gemm.cpp','mix.cpp','common.h','norm','gemm','mix','norm-build.log','gemm-build.log','mix-build.log']:
 f=old/name;h=sha(f);assert h==m[name];shutil.copy2(f,p/name);assert sha(p/name)==h;files[name]=h
for name in ['norm','gemm','mix']:assert (p/(name+'-build.log')).stat().st_size==0
...
(root/'.q4t-work/e2e/moe-scale-hc-read-l2-20260924/reference').mkdir(parents=True)
