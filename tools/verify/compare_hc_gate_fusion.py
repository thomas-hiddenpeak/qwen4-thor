...
def main():
    ...
    baseline = subprocess.check_output(
        ['git', 'show', f'{args.baseline_ref}:src/model/gemv_hc_inject.cu'],
        cwd=ROOT, text=True)
    old_hc = subprocess.check_output(
        ['git', 'show', f'{args.baseline_ref}:src/model/hyperconnection.cu'],
        cwd=ROOT, text=True)
    current_hc = (ROOT / 'src/model/hyperconnection.cu').read_text()
    if current_hc != (run / 'hyperconnection.cu').read_text():
        parser.error('HC source differs from accepted snapshot')
    helpers = old_hc[old_hc.index('__device__ __forceinline__ float Bf16ToFloat'):
                     old_hc.index('// GroupedGemmaRMSNorm:')]
    gate_kernel = old_hc[old_hc.index('__global__ void ApplyInjectGateKernel'):
                        old_hc.index('// Fused inject-gate + combine:')]
    original_kernel = helpers + gate_kernel + baseline[
        baseline.index('__global__ void HcInjectGevKernel'):
        baseline.index('}  // namespace')]
    ...
    (out / 'original-hc.cu').write_text(old_hc)
    (out / 'candidate-hc.cu').write_text(current_hc)
    ...
    if result['cases'] != 2048 or result['values'] != 8192 or result['mismatches'] or result['nonfinite']:
