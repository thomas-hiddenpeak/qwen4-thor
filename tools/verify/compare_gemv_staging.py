...
def main():
    ...
    candidate = (ROOT / 'src/model/gemv.cu').read_text()
    if candidate != (run / 'gemv.cu').read_text():
    ...
    candidate_kernel = candidate[candidate.index('__device__ __forceinline__ void F32x2Fma'):
                                 candidate.index('// FP8 weight quantization:')]
    ...
    if result['cases'] != 2016 or result['values'] != 105408 or result['mismatches'] or result['nonfinite']:
