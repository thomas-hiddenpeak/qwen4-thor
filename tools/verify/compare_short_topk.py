"""Compare short-context selection slots after full HTTP acceptance.

This is an exact selection check, not a benchmark. The same FP32 scores exercise
dense/sparse boundaries, ties, signed zero and mixed-position prefill rows.
...
def kernel_source(source, header, namespace):
    begin = source.index('__global__ void TopkSelectKernel(')
    end = source.index('\n// ---------------------------------------------------------------------------', begin)
...
def main():
    ...
    after = (ROOT / 'src/model/short_topk.cu').read_text()
    if after != (accepted / 'short_topk.cu').read_text():
    ...
    fixture = Path(__file__).with_name('short_topk.cu.in')
