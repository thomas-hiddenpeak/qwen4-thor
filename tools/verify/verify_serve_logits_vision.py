"""Compare real image/video tower inputs through full-model head modes."""
import argparse
import hashlib
import json
...
def main():
    ...
    for filename in ['vision_test_a.png', 'vision_test_b.png']:
        shutil.copy2(run / 'consumers-vision' / filename, out / filename)
    for name in ['image', 'two-images', 'video']:
        payload = json.loads((run / 'consumers-vision' / (name + '.jsonl')).read_text())
        ...
        for part in message['content']:
            if part['type'] == 'text':
                ...
                assert part['type'] == 'video' and len(part['video_frames']) == 2
                assert part['video_frames'][0] == part['video_frames'][1]
                prompt += '<|video_pad|>'
        (out / (name + '.txt')).write_text(prompt)
        rows = json.loads((run / 'consumers-vision' / 'candidate' / name / 'parsed.json').read_text())
        assert len({x['prompt_tokens'] for x in rows}) == 1
        (out / (name + '.tokens')).write_text(str(rows[0]['prompt_tokens']))
