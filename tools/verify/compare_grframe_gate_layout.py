"""Check independent GRFrame gate region and compiled capacity after full HTTP."""
import argparse
import hashlib
import json
...
int main() {
      ...
      for (bool full : {false, true}) {
        for (bool ple : {false, true}) {
          for (int lowrank : {160, 320, 640}) {
            ...
            Require(now.gate == old.total_bytes,
                    "gate appended after old arena");
            Require(now.gate_bytes == size_t(t) * 4 * 2, "gate logical bytes");
            Require(now.total_bytes ==
                        old.total_bytes + baseline::AlignUp(now.gate_bytes),
                    "gate capacity growth");
            ...
            const std::array<std::array<size_t, 2>, 10> regions = {
  ...
  for (int kind = 0; kind < 3; ++kind) {
    const auto old = baseline::MakeDecoderWorkspaceLayout(
        8192, kind == 2, kind == 1, 4, 2560, 512, 640, 640, 10, &weights, 320);
    const size_t now = DecoderLayerWorkspaceBytes(
        8192, kind == 2, kind == 1, 2560, 512, 640, 640, 10, &weights, 320);
    before = std::max(before, old.total_bytes);
    after = std::max(after, now);
  }
  std::printf(
      "PASS: %d layouts; %d linked capacities; independent gate region\n",
      cases, cases * 2);
  std::printf("model workspace T=8192: %zu -> %zu, added %zu bytes\n", before,
              after, after - before);
}
"""
