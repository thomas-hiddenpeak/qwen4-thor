...
归档后发现初始manifest早于最后一行stdout，只有run.log末态
摘要变化，原前缀仍匹配。已使用qsa_full_attention_fp64/finalize.py
模板对应工具逐项核对并追加final-artifact-binding.json，原清单及
文件保留。此处run.py已将最后打印移至初始清单之前；本次实际
执行源码仍在证据目录。最终封存应在所有写入进程退出后执行。
