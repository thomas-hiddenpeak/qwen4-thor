...
v7六个HTTP原始响应、提示词摘要、参数、MTP关闭、实际长度及
7token/stop已核对；服务日志disable_radix_cache=True，六次
cached-token均0。1K三次360019；4K为600430/600430/360284，
仍只有1/6正确。服务再次在排空请求后自行kill_process_tree，
退出-9。证据independent-sglang-http-v7-20260923/final-raw-audit.json。

这排除了“只要关闭跨请求前缀复用就消失”的解释，不能据此
定位QSA、MoE归约、其他数值路径或编译警告为根因。两轮独立
框架1K共六次同候选错误，但4K本身不稳定，不能充当唯一真值。

下一项有界复核v8仅加--enable-deterministic-inference，保持
前缀缓存关闭和原六请求，已启动会话95305。它可能改变独立
框架内部后端策略，须核对启动日志；仍不是项目候选验收，
不扩展为修复独立框架kernel的任务，不改变原正确答案。


## 确定性模式有界复核结果与阶段结论

v8日志确认enable_deterministic_inference=True、sampling=pytorch、
Triton attention、CUTLASS MoE、MTP关及radix cache关。首个
1024-token真实HTTP forward进入batch_invariant_ops后调用
DeepGEMM bf16_gemm_nn，报Unsupported architecture，形状
M=1024/N=16480/K=2560；错误不是矩阵太小的证据，外层通用
提示不能覆盖底层架构不支持原因。服务走崩溃清理后-9退出。

SQLite六行全部success=False、输入输出token均空，0个有效
答题样本。驱动exit中的completed=6是记录行数，不是成功数，
不得称为0/6答对。证据independent-sglang-http-v8-20260923/
failure-audit.json。当前所有参考服务均已终止，不继续改参考
框架kernel或关闭安全检查，不把安装/启动尝试计入精度统计。

| 版本/设置 | 1K期望360010 | 4K期望360284 |
|---|---|---|
| 已接受7364767（原15题矩阵） | 360010 | 600440 |
| 当前候选33b11796（原15题矩阵） | 360019 | 600430 |
| 独立SGLang，缓存开，各3次 | 360019×3 | 600430 / 360284 / 600430 |
| 独立SGLang，缓存关，各3次 | 360019×3 | 600430 / 600430 / 360284 |
| 独立SGLang，确定性选项 | HTTP执行失败，无答案 | 连接失败，无答案 |

阶段结论：独立完整模型的1K六次均与候选同错，结合位置源码
合同，不能再简单把这个单题变化称为本项目独有的数学错误。
但原独立答案矩阵的14/15→13/15仍是实测回退，没有被消除；
独立框架的4K也不稳定，且量化合同有差异，因此它不够资格
单独裁定候选正确。当前不接受候选、不重置答案/输出参考，
不声明“精度已保证”。本阶段新增12个有效独立HTTP响应、
6个失败传输记录；之前54个项目/隔离响应全部保留。

下一步需要明确区分两个验收问题：位置/计算是否符合模型定义，
以及相同任务集质量是否保持。现有证据只加强前者的可信度，
没有满足后者和完整性能条件。恢复性能开发前，应先把候选
拆成独立改动，并建立能覆盖这些缺陷的稳定E2E质量判据；不得
为了推进而豁免现有失败样本，或以参考框架同错替代质量通过。


## 恢复版独立答案确认终态

恢复源码与7364767一致、二进制2a5813f2重新完成原冻结五档15题：
14/15，15条全文均同旧正式，新增失败0。1K求和恢复360010；
4K仍600440，应为360284。质量评估器退出1、服务0，外层原始
证据核对退出0，不将采集成功当成质量全通过。本轮有界恢复
确认结束，未解决全模型数学正确性；详见
[恢复确认报告](CORRECTNESS_CONFIRMATION_2026-09-23.md)。
证据accepted-workspace-recovery-answers-20260923，聊天入口未跑。
