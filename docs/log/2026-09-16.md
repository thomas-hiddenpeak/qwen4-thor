...
## 2026-09-16 — prefill roofline 分析: 算力仅用 ~1-4%, 瓶颈是访存+延迟不是算力

**动机**
用户质疑"prefill 是算力受限的活儿, 之前都当成访存受限, 是不是算力没用满、有大量
优化空间?"。用严格 roofline 回答 (tools/roofline_prefill.py), 而非继续假设。

**测量 (硬事实)**
- 整个 prefill (2560 tok) 总算力 = **28.2 TFLOP** (由真实维度算出)。
- 达到 = 28.2 / 2.6s = **10.8 TFLOPS = FP4 峰值的 1.0% / BF16 峰值的 4.2%** →
  **张量核 ~96% 时间在空转**。
- 纯算力下限 (FP4 峰值) 仅 **0.03s**, 实测 2.6s = **86× 算力下限** → wall time
  几乎全是访存搬运 + 延迟停顿, 不是算术。
- 硬件计数器 (ncu / nsys gpu-metrics) 需 root (ERR_NVGPUCTRPERM), 本机无法取,
  故用 FLOPs+实测时间的 roofline 替代 (结论对峰值取值不敏感, 2× 误差仍 <10%)。

**为什么"prefill 算力受限"直觉在本模型不成立 (逐分量 AI)**
| 分量 | GFLOP | AI (FLOP/B) | vs ridge | 界 |
|---|---|---|---|---|
| MoE routed | 12080 | **200** | FP4 ridge 4295, 低 21× | 访存 (权重带宽) |
| attn core (sparse) | 1546 | 低 (KV 复用差) | — | 访存 (KV gather) |
| GDN 递归 | 290 | — | occupancy 25% 上限 | 延迟 |
| dense 投影/MoE shared | — | 2560-10240 | > ridge | **算力** (已近峰值) |

根因: **MoE 把一个 dense FFN 拆成 512 个专家 × ~50 token/专家的小 GEMM** →
每个专家权重 (1.6MB fp4) 只摊到 ~50 token → AI=4×50=200 ≪ ridge → 权重带宽受限。
dense transformer 的 prefill 才是算力受限; MoE + 稀疏/线性注意力使本模型即便
prefill 也访存/延迟受限。

**经验标度曲线 (证据)**
T=1280→1012.7 / 2560→989.4 / 5120→950.7 tok/s — **随 T 增大 tok/s 反而略降**。
若受固定权重加载摊销限制, 大 prefill 应更快/token (权重被更多 token 摊薄); 实测
持平略降 → 主导成本是**随 T 增长的 per-token 访存** (稀疏注意力 KV gather), 不是
可摊销的固定算力。

**结论 / 优化方向重定位**
- 算力确实远未用满 (~96% 空转) — 用户直觉对; 但**空转是访存受限的"症状"**, 不是
  可用"更多算术"填满的浪费。整个 prefill 的算术在张量核峰值下只需 0.03s。
- 真正的空间在三处 (都是减少访存/延迟停顿, 非加速算术):
  1. **减访存**: 稀疏注意力 FP8 KV (KV 字节减半; 但 vllm 主 attn 仍 bf16, 质量风险)。
  2. **提占用/减停顿**: 让延迟受限 kernel 逼近访存屋顶 (SparseAttn 已 71% BW /
     GDN occupancy 25% 上限, 近地板; 空间有限)。
  3. **批处理 (最大乘数)**: 单请求 MoE 每专家仅 ~50 token (AI 200); 连续批处理
     把并发请求 token 打包 → ~400 token/专家 (AI 1600) → 逼近算力受限 → 同样权重
     摊到 8× token → 聚合吞吐倍增。**这才是填满空转张量核的正道** (vllm 1k+ 多半
     是高并发下的聚合数; 我们单请求 989 已具竞争力)。
- 即: 单请求 prefill 已近其结构性天花板 (~1000 tok/s), 单 kernel 微优边际递减;
  数量级提升靠 Phase 2 连续批处理的**聚合吞吐**, 不靠单请求算术加速。

**下一步**
方向由本分析确定: 要么 monolithic MoE (收回 MoE ~53% 带宽效率的一半, 单请求
~8%), 要么转向连续批处理 prefill 打包 (聚合吞吐, 数量级)。工具留 tools/
roofline_prefill.py 可复算。

## 2026-09-16 — HC combine inject 门控预计算 (去 hs 倍冗余 sigmoid) → prefill 981→989 tok/s

**做了什么**
`CombineWithGateKernel` 的 inject 门 `gate = 2·sigmoid(inject_raw[t,b]/hc)` 只依赖
`(t,b)`, 与通道 c 无关, 但融合版在每个 `(t,b,c)` 线程里**重算一遍** → hs=2560 倍
冗余 sigmoid。nsys 佐证: CombineWithGate 仅 ~57% 带宽, 而访存模式几乎相同的 MixGate
达 ~85% — 差距即这层冗余算。改为 `ApplyInjectGateKernel` 在 inject GEMM 后就地把
d_inject[T,hc] 预计算成门值 (每 (t,b) 一次 sigmoid), combine 直接读预计算门。

**为什么 bit-exact**
预计算走与原融合版**完全相同的 BF16 round-trip** `FloatToBf16(2/(1+exp(-v)))`,
combine 读 `Bf16ToFloat(inject_gated)` = 原 in-register 值逐位相同。

**结果**
- prefill 2560 tok: **981 → 989 tok/s (+0.8%)** (2 次 989.0/989.7)。
- nsys: CombineWithGate **244→211ms** (med/call −13%), ApplyInjectGate 仅 +0.55ms。
- HC 测试 combine l2_rel 2.330e-3 不变。全 68 测试通过零警告。

**本 session prefill 累计**: 826→871→893→947→981→**989 tok/s** = **+19.7%**,
vllm 1k+ 的 ~0.99×。

**下一步**
CombineWithGate 仍 ~65% 带宽 (< MixGate 85%), 剩余差距为 block_output 广播读模式
(每 (t,b,c) 读 block_output[t,c], 跨 b 广播), 属访存模式固有, 收益递减。

## 2026-09-16 — MoE combine 批量化 (per-expert scatter → 单次确定性 gather) → prefill 947→981 tok/s

**做了什么**
MoE 路由前向每个专家单独 launch 一次 `ScatterAddKernel` 把该专家的 down-GEMM
输出散射回 y。nsys: **29178 次 launch / 5.2% GPU 时间 / med 3.5µs (低于 ~5µs
launch 开销 → launch-overhead 主导)**。改为:
1. down-GEMM 输出写入**专家分组的连续 [R, hs] 缓冲** `ws.dn_out + offset[e]*hs`
   (offset = counts 前缀和; `beta=0` 各专家覆盖各自不相交切片);
2. `BuildRowOfFlatKernel` (1 次 launch) 建 `row_of_flat[flat] = 分组行号` 逆映射;
3. `CombineGroupedKernel` (1 次 launch) 每线程一 (token t, 通道 c): 沿 k 个 slot
   `Σ router_w·dn_out[row_of_flat[t*k+slot], c]` 写 `y[t,c] += acc`。

**为什么用 gather 而非批量 atomicAdd scatter**
批量 scatter 会让同一 token 的 k 个专家行**并发 atomicAdd 竞争 y[t]** (旧 per-expert
串行 kernel 反而无竞争)。gather 版每个 `y[t,c]` 由唯一线程写入 → **无原子、确定性、
且 y 流量从 R·hs 降到 M·hs**; warp 内同 slot 共享行号, dn_out 读跨 c 合并。

**结果**
- prefill 2560 tok: **947 → 981 tok/s (+3.6%)** (2 次 980.8/981.5, 低方差)。
- nsys: MoE combine **287ms → 87ms GPU 时间** (CombineGrouped 86ms + BuildRowOfFlat
  0.88ms), launch **29178 → 144** (−99.5%)。
- 数值**逐位不变**: routed_forward max_rel 0.0077468 / e2e l2_rel 0.0098645 与改前
  完全一致。全 68 测试通过零警告。

**教训 (nsys 陷阱)**
先只重建了 `q4t_tests` 忘了重建 `q4t` 主程序, 首测 prefill 947 无变化 + profile
仍显示 ScatterAddKernel → 误判"launch 开销被异步重叠掩盖"。**重建正确 binary 后
+3.6% 兑现**。教训: `generate` 用 `q4t`, 优化 MoE/kernel 后必须 `--target q4t`。

**本 session prefill 累计**: 826 → 871 → 893 → 947 → **981 tok/s** = **+18.8%**,
已达 vllm 1k+ 的 ~0.98×。

**下一步**
GatherQuant (4.8%) / SwiGLUQuant (3.8%) 仍 per-expert 29353 launch, 但 med
5.9/6.1µs > launch 开销 (做真实量化工作, 非纯 launch-bound) 且 per-expert SF
swizzle 需 128 行对齐, 批量化收益小且受阻。剩余大杠杆仍是 MoE 单体 FP4 kernel。

## 2026-09-16 — QSA indexer logits 张量核化 (SIMT 三重循环 → Bf16Gemm) → prefill 893→947 tok/s

**做了什么**
`IndexerLogitsKernel` 是 `logits[t,g]=Σ_h relu(iq[t,h]·ck[g])/√hd` 的 **SIMT
三重循环** (nsys 5.7%, ~9-14ms/call, ~60 GFLOP/s, 张量核峰值的 ~0.2%)。它本质
是个 GEMM `iq[T·n_iq, hd] @ ck[n_groups, hd]ᵀ`。改为**张量核 `Bf16Gemm`** 算
S[T·n_iq, max_blocks] + 小 reduce kernel (per-head relu + Σ_h + 1/√hd + 因果
group mask)。仅单序列路径 (d_seq_id==nullptr) 走 GEMM, 多序列 (per-seq idx_comp
切片无法共享一个 GEMM 权重) 保留 SIMT。

**为什么 N=max_blocks 无需 base position**
GEMM 算全部 max_blocks 列 (含未建 group 的 idx_comp 垃圾行), 但 reduce 对
g >= n_groups(t) 一律写 -1e30 **不读 S**; 而 g >= n_groups_max 对所有 t 都被
mask, 故垃圾列的 S 永不被读 (即使是 NaN 也无害, cuBLASLt 不崩)。省掉 base
position 的 host 读取/plumbing。

**结果**
- prefill 2560 tok: **893 → 947 tok/s (+6.0%)**。full_attention 测试 (稠密退化
  区, T=8) out l2_rel 4.688e-3 不变 (稠密下 top-k 选全部, logits 精度不影响
  输出); 稀疏区 (T=2560>2048) 生成文本连贯 (正确续写重复 prompt)。全 68 测试
  通过零警告。
- S 暂存 buffer T·n_iq·max_blocks·2 (max_prefill=8192 时 134MB, 122GB 内存下
  可接受)。bf16 dot (vs 原 f32) 在稀疏区 top-512 选择或有近平局翻转, 属 bf16
  噪声, 稠密区 (≤2048) 逐位不变。

**本 session prefill 累计**: 826 (Step 10) → 871 (MoE compact) → 893 (MoE bf16)
→ **947 (indexer 张量核)** = **+14.6%**, 已在 vllm 1k+ 的 ~1.06× 内。

**下一步**
剩余大杠杆是 MoE 单体 FP4 kernel (flashinfer, 大改)。attention/GDN 近地板。
