...
## 2026-09-24后续：完整卷积算术参考

复用上一阶段冻结HTTP，不改变生产或观测器。参考零警告构建后，
绑定12份完整操作数/输出并直接重读checkpoint卷积权重。按最多
128行分块，覆盖完整4096 prefill及首decode，共41953280个值。

| 参考 | prefill差异 / 41943040项 | 首decode差异 / 10240项 |
|---|---:|---:|
| CPU四项FP32 fma + 设备仅SiLU | 0 | 0 |
| CPU四项FP32 fma + CPU expf | 23 | 0 |
| FP64卷积与稳定SiLU | 75 | 0 |

三种参考均保留卷积激活、PLE加法、主干加法三次BF16舍入。
差异均非仅零符号不同；不能把数学函数/累加参考的位差自动判成
新增缺陷。设备没有重放生产卷积，只接收CPU累加值计算SiLU。

33块全部参考先完整落盘，再以实际基底+delta逐字节/SHA恢复后
删除临时BF16；最后独立重建六个完整phase/variant并确认连续
覆盖。保存全部CPU FP32累加/设备SiLU，以及FP64最终输出差异
位置的累加/SiLU与索引；其他未舍入FP64不落盘。融合内部值
没有直接观测，不能把最终输出一致扩张为内部状态全同。

证据：`.q4t-work/e2e/ple-full-conv-reference-20260924/`；模板：
`tools/verify/ple_full_conv_reference/`。参考后可用21947273216字节。
...
## 2026-09-24后续：完整归一化与门值算术

复用已完成三侧HTTP的冻结快照，生产/观测器未改。参考零警告
构建后，直接重读checkpoint三组权重，绑定全部操作数，128块
覆盖全部4096位置的三组norm和gate。

| 参考 | norm差异 / 125829120项 | gate差异 / 16384项 |
|---|---:|---:|
| CPU指定运算，设备仅数学函数 | 0 | 0 |
| 同运算顺序的纯CPU数学函数 | 225 | 0 |
| FP64参考，经FP32/BF16舍入 | 1284 | 2 |

三组norm的CPU差异分别55/112/58，FP64分别802/284/198。
FP64 gate差异为零起始token840、3473的branch1，实际值分别
0.00244140625、0.00128936767578125，参考舍入值分别
0.0024566650390625、0.0012969970703125。均非仅零符号不同，
不因这两项位差直接推断原错答原因。

norm由CPU按八值平方FMA、每lane十组累加和32lane XOR重建，
设备只提供rsqrt；gate由CPU顺序2560次FMA乘实际scale，设备
仅提供开方/符号/sigmoid。纯CPU使用double sqrt/exp后转换至
指定FP32边界；FP64保留实际eps/scale。三种gate参考均使用
实际norm输出，不传播高精度norm，属于条件算术参考。

所有完整BF16结果以基底+delta逐块逐字节/SHA恢复，末尾重建
12份完整kind/variant并确认连续覆盖；所有数学函数FP32/FP64
参数、设备结果和FP64最终差异处未舍入值均保留。一致位置未
舍入FP64输出不落盘，融合内部实际累加值未直接观测。

证据：`.q4t-work/e2e/ple-full-norm-reference-20260924/`；模板：
`tools/verify/ple_full_norm_reference/`。100MB预算内完成，保留
20GiB磁盘余量，归档后可用21666934784字节。原错答仍在，下一步
完整key/value投影与embedding交接，随后补完整QSA各query。
