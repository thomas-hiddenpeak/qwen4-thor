...
| MoE路由 | 完整HC MLP mixed、router权重/算法/logits、ID及路由权重 | 投影重放/FP64、Top10与softmax | 原4K48层全位置已核对；FP64舍入后103行集合不同，未建任务因果；全行mapping/gather/专家量化与计算仍待补齐 |
...
不能覆盖任务质量退化。下一步接通完整prefill MoE路由的mapping/gather/输入量化，再补专家计算及QSA/PLE链，
