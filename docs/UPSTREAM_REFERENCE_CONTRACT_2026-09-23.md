...
| QSA cache/indexer | 位置、RoPE、KV页表/新增槽、压缩索引、分数和top-k | 因果可见集、索引选择、softmax及attention | 12层全部4096 query的选择、KV及attention指定设备原语+CPU参考已核对；全query高精度及显式舍入差异已完整保留，主投影及norm/RoPE全量参考已补齐，完整indexer评分链仍待补齐，压缩/indexer旧参考范围不能扩张 |
...
不能覆盖任务质量退化。完整indexer矩阵/汇总及压缩输入复用已核对，下一步补查询变换，
再核对组合前向，
