...
最新完成：四分支layer2 HC七阶段及QKV/Z/A/B投影，原支全部恢复。
新mixed与四投影分别影响4079/4094/4094 token；注入门值已保存。
短卷积/GDN尚未计算。复用冻结通用helper并重绑本层权重/算法，
当前约24GiB空闲，下一阶段先核对证据共享容量，保留20GiB余量。
...
1. 四分支已到layer2 QKV/Z/A/B；先核对证据共享容量，再接
   短卷积/qk norm/GDN及后续MoE，保留20GiB余量及全部冻结证据。
