...
最新完成：四分支layer1 GDN q/k归一化、A/B门值及4096步递推，
原支全部恢复。输入/中间/双项修正影响4079/4094/4094个Y位置，
末态分别改变443655/786430/786430个F32。变化符合因果支持集；
尚未接NormGate/输出与HC写回，不能由传播数量判断最终答案质量。
...
1. 四分支已过layer1 GDN递推；接NormGate/输出，使用各支自身
   Z、inject gate和PLE主干，再做HC写回及MoE。
