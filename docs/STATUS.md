...
- **原生次正规转换候选e49bc64f已启动首项HTTP。** device改用
  最近偶数整数转换，host保留手写逻辑，高端缺陷未改。必要构建0、
  零警告，固定11题已原始核对11/11且全文同正式、服务0；会话
  63732已结束，独立14/15且全文同父版、新增失败0，原4K错保留。
  本版完整五档性能会话83189已启动，未接受或提交，低层测试未运行。
...
`.q4t-work/e2e/short-topk-register-20260921/`。build/q4t 当前为已接受模板修复2392f6b1，验证边界见顶部摘要；
