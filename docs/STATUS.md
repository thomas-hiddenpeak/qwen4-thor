...
已核对原始完整终态后启动预定相邻五服务75请求，PID2270980
确认存活：正式→候选→固定→候选→正式。证据
`decode-state-staging-adjacent-20260923`；驱动/日志在同名prepared目录。
逐项保留全部60个候选/参考/档位/指标比较及历史失败，首请求
单列，不合并范围，不改变各二进制输出参考。尚未完成，
该性能诊断不能解除候选与正式输出不同的精度条件。
