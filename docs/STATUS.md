...
- **当前未接受模板候选为2392f6b1。** 移除全局Json额外字段，数字原文
  改为仅HTTP/工具参数解析时按需保存到数字类型未用的str中；配置解析
  默认不保留。上一版5e6d2ee6源码/二进制/失败记录已归档。新构建0且
  零警告，首项24条evalscope HTTP已通过入口对照，messages六组均正确
  且同原生。扩展45条全部差分一致（其中独立答案15/15）；固定质量
  11/11已原始核对且全文同正式，五档15条输出同正式、服务0。
  完整三次范围对正式/固定/恢复均0不利；后两次对恢复0，历史正式4K TTFT
  与固定8K decode仍不利。非法输入20条通过，视觉72条输入/输出一致；
  视觉48项比较9项单侧不利、0项同时差于前后参考，参考自身后组有3项
  decode下降，原始记录均保留。独立15题运行中（会话92721），候选未接受。
...
- 已接受运行时仍为 **7364767**，固定性能参考仍为 **fcb5925**。
  九个自有实验源码归档并恢复，src/include/CMakeLists与7364767一致，
  恢复阶段运行时无差异；现有模板候选见上，其他任务文档保留。
  恢复零警告构建二进制2a5813f2，
...
`.q4t-work/e2e/short-topk-register-20260921/`。build/q4t 已重建为2a5813f2，质量及五档输出通过，后续子集性能诊断待闭合；
...
均已接受，后者为7364767，见
[容量报告](../dataflow-engine/SERVE_LOGITS_CAPACITY.md)。当前先完成
恢复版本验证，再针对标准messages与原生模板prompt做HTTP对照：
现server只拼接role与内容，固定质量题预渲染模板不能覆盖该入口。
