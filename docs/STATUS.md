...
- 已接受运行时仍为 **7364767**，固定性能参考仍为 **fcb5925**。
  ...
  新二进制2a5813f2不同旧归档；首项固定质量HTTP11/11原始
  核对通过、输出同正式、服务0。五档性能会话41428进行中。
...
- 全部失败源码、补丁、二进制与HTTP证据保留。下一步收齐恢复
  版本五档性能并比较双参考，不能将质量11/11当全面正确性证明；
...
`.q4t-work/e2e/short-topk-register-20260921/`。build/q4t 已从接受源码重建为2a5813f2，恢复后E2E待完成；
