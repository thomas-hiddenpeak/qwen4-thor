...
- 已接受运行时仍为 **7364767**，固定性能参考仍为 **fcb5925**。
  当前build/q4t与九个未提交运行时文件属于混合候选，不能当正式版。
- 混合候选未接受：独立答案正式14/15、候选13/15，新增1K求和
  错误可由位置补写单独触发；源码合同支持补写，独立框架同题也
  答错，但这些都不能抹除已观察到的质量回退或证明全模型正确。
- 单独提取的M2–4投影版本41a2a02f未新增已测质量错误：固定
  11/11、独立答案14/15且全文同正式、并发60/60、尾块18/18。
  原4K求和错误保留。完整五档15条输出同正式，但7项双参考
  性能范围不利，当前未接受，未开始低层测试或profile。
- 当前只运行相邻HTTP复核：正式→独立投影→正式，1K/4K/8K/
  200K各三次，共36请求，会话24545，证据
  small-m-isolated-adjacent-20260923。结果未收齐，不能提前通过。
