...
- **原生次正规转换候选e49bc64f尚未接受。** 固定质量11/11，
  独立14/15且15条全文同父版，无新增失败、原4K错误保留。
  自身五档15条原始HTTP核对完成、全文同三个参考、服务0；全三次
  范围0项不利，后两次对父版/正式/固定4/1/2项不利，不能据此
  关闭性能疑点。已启动一次固定父版→候选→父版五档45请求对照，
  控制器PID2442767，证据e4m3-subnormal-native-adjacent-20260923。
  不改变参考或容忍比例；低层测试未运行，高端编码缺陷仍未修复。
...
`.q4t-work/e2e/short-topk-register-20260921/`。build/q4t 当前为未接受候选e49bc64f，已接受模板二进制2392f6b1
保存在prepared/e4m3-subnormal-native-20260923/q4t-before，见顶部摘要；
