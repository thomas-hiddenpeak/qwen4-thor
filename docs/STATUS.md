...
最新完成：首层输入/中间高区间尺度修正四分支已到MLP block。
只修输入、中间、双项分别改变860418/2478211/3287910个最终
BF16值，涉及378/1185/1535 token；单项交集28，双项影响恰为
并集。原分支逐位恢复。尚未经过HC写回/PLE或进入layer1，不证明
质量改善。详见 [尺度传播](MOE_SCALE_PROPAGATION_2026-09-24.md)。
...
1. 四分支已到首层MLP block；接HC残差写回/PLE边界，先核对
   原分支恢复，再传播变化至后续层。
