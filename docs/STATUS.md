...
最新完成：四分支layer1新路由分组、输入量化与GU。原支按专家/
token身份全部恢复，新行数均兼容记录算法；新增147/212/238
专家行单独保留。共有输入不变而输出变化的行数为0。尚未中间量化、
down与共享分支；本链仅传播layer0干预，layer1仍保留原尺度缺陷。
...
1. 四分支已到layer1新路由GU；接自身中间量化/down，重算共享
   分支，按新flat slot/权重合并，再用自身MLP门值写回。
