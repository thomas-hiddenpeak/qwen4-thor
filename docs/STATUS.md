...
最新完成：输入尺度单项修正已传播到首层MLP block出口。
原分支中间量化/合并逐位恢复；修正分支中间SF改变43066项、
FP4改变359492项，最终MoE BF16改变860418项，均限于原378个
受影响token。尚未经过HC残差写回或进入layer1，不证明质量改善。
...
1. 输入尺度单项已到首层MLP block；补中间尺度单项及两处同时
   修正的对照，再接HC残差写回/PLE边界和后续层。
