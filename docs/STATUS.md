...
同一二进制已启动 `indexer-mixed-topk-batches-http-20260922`，
PID 1901062，复验 1K/8K 各三轮 B=2/3/4 和串行前后共 66
实际 HTTP，仅 host 日志，无额外诊断 D2H。运行时代码未改。
下一步核对大批量和 8K 的完整 token 链；正式质量与五档性能
仍需最终二进制验证，没有运行前置底层测试或 profile。
当前候选二进制 SHA-256：
`fee26e7f04c54a58e664de021276c8909e4c5996ce9e420e2c3a4403dfe2979c`。
不同 prompt、批量 prefill、完整五档性能及其他消费者仍待验收。
