...
- **当前有未接受模板候选5e6d2ee6。** 新增模型专用聊天渲染，含思考、
  历史、工具及视觉边界；工具JSON保留整数原文。零警告构建后首项
  为24条evalscope HTTP，messages六组均同原生模板且6/6正确，
  原生前后各6/6、旧拼接仍0/6。45条扩展HTTP全部差分一致，
  其中有独立答案的15条正确。固定11题已原始核对且全文同正式；
  五档15条已原始核对且输出同正式，服务退出0；完整三次范围对正式/固定/恢复
  均无不利分离，后两次子集分别3/2/6项不利，性能疑点保留。18种非法输入
  均预期400，前后健康请求均正确；视觉72条长度/全文/结束一致，
  但视觉时延48项比较中8项不利（部分decode同时低于前后参考），
  四服务已退出0。独立15题回归运行中（会话71505）。候选不接受。
