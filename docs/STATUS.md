...
最新完成：首层输入尺度修正到GU的局部传播。3800组高区间尺度
修正涉及378个token、3780条专家行，改变4567449个GU BF16值；
输入未变行输出全同。传播尚未越过GU，不能推断最终答案改善。
详见 [尺度传播](MOE_SCALE_PROPAGATION_2026-09-24.md)。完整indexer
条件参考已补齐，仍非独立整模型证明。
...
1. 首层输入尺度→GU已完成；继续SwiGLU/中间量化（先保留原
   编码规则）、down和合并，核对原输入恢复与修正后首层出口。
