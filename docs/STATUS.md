...
当前运行时代码未接受、未提交；不能把实验采集成功当作验收通过。
完整过程、旧候选结果与失败保存在
[decode 路径报告](DECODE_PATH_CONSISTENCY_2026-09-21.md) 和开发日志。

当前二进制 SHA-256：
`a95dbafdeb94e885b91f496d155d133844fd049992f1fd7d518cf17a5aacbc31`。
它包含此前 RoPE/打包 GDN/BF16 小行投影/混合索引修正，以及新增的
共享 FP32 GDN 与 checkpoint 编译期分派。层/阶段诊断开关仍在，
正式质量和性能采集时关闭；插桩时间不用于性能结论。

### 已定位的问题与本轮修改

- 正式父版自身存在串行/并发输出分叉；此前候选已在同 prompt
  B=2/3/4 和混合长短上下文 HTTP 中恢复路径一致，但这些旧结果
  **不能自动转移为当前共享 FP32 候选的验收**。
- 普通 decode/MTP 的实际 HTTP 对照，在 8K/44K 首个 decode 步
  第 0 层出现差异。初始 trunk/SSM/conv 摘要相同，投影与卷积
  输出相同，首次不同阶段为 GDN。两轮各 24 HTTP 的诊断开关
  均保持各模式输出；哈希不证明精度，也不证明后续层只有此问题。
- register 路径原先把归一化 q/k 写回 BF16，causal 路径保留 FP32，
  归约/更新顺序也不同。当前目标 kd=vd=128 的普通、打包、因果/
  ragged 路径共用 register 递推，保留 FP32 归一化因子，checkpoint
  按实际 slot/local token 保存。这是待验收算术变更。
- 因子放在 qkv_raw 分配的对齐尾部，不覆盖原投影，无新增分配
  次数，容量增加 8*T*nkh 字节加最多 6 字节对齐。初版保留逐
  token checkpoint 判断，出现大幅 TTFT 回退；当前将 checkpoint
  需求在 host 分派为编译期参数，递推算术与分配布局保持初版。

### 当前证据与未通过项

必要构建零警告，首项正式质量 HTTP **11/11**（含 200K），完整
输出同 7364767，服务/驱动退出 0。没有前置底层测试/profile。
完整性能 **15 请求采集有效**，原始 SQLite 响应与公式已核对，
服务/驱动退出 0；但正式输出和部分性能门禁仍未关闭。

| 输入 token | 当前 TTFT 秒 | 当前 decode tok/s |
|---:|---:|---:|
| 1024 | 0.802 | 18.748 |
| 4096 | 2.643 | 18.064 |
| 8192 | 5.188 | 18.045 |
| 45056 | 30.554 | 17.822 |
| 204800 | 159.832 | 17.179 |

五档均为单流/MTP 关闭、每档三次、输出 256 的均值。完整输出均
同共享 FP32 初版，**仍不同于正式参考**；质量题通过不能覆盖
该差异。8K/44K TTFT 对正式父版不利范围分离，44K 对前一候选
也不利；其余档位与各档 decode 无不利分离。相对 FP32 初版的
改善不能抵消这些未通过项，也不构成稳定提速结论。

证据：gdn-checkpoint-specialization-20260922（质量）、
gdn-checkpoint-specialization-performance-20260922（完整矩阵）。
初版 gdn-shared-fp32-performance-20260922 的 4K/8K/44K/200K
TTFT 回退、此前候选的正式输出/MTP 差异均保留，不替换样本。
固定 Transformers 参考仍为 5.16.1；evalscope 环境 5.17.0 的
归一化顺序不同，不能混用，尚未执行本轮参考数值验证。

### 当前执行与下一步

8K/44K 旧→新→旧相邻 HTTP 正在运行：
gdn-checkpoint-parent-pair-20260922（PID **1981749**），三侧每档
三次，共 18 请求。各版检查自身原输出，原始矩阵与失败保留。
随后执行已准备的共享 GDN MTP/阶段 HTTP，检查接受/回滚和
首层差异是否消除；此前的批处理/回退/视觉结果需要按改动影响
重新核对。正式输出与精度、相邻性能和 MTP 结果全部未闭合前，
不接受、不提交运行时，不开始底层数值测试或 profile。
