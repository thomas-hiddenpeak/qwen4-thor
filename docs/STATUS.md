...
- **次正规分项候选43b0e688已启动HTTP隔离。** 只改a<2^-6的
  最近偶数与进位，高端编码缺陷继续保留，不宣称完整格式修复。
  构建0、零警告，首项固定11题HTTP已原始核对11/11、全文同正式，
  服务0；独立15题已核对14/15、全文同父版、新增失败0，原4K错保留。
  完整五档HTTP性能已启动（会话44477），低层测试未运行、无自动
  接受。父版2392f6b1已归档，当前候选
  未接受、未提交，见[编码审查](E4M3_ROUNDING_2026-09-23.md)。
