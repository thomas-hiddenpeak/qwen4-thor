...
最新完成：四分支layer1 MLP HC七阶段和新路由，原支全部恢复。
输入/中间/双项修正分别使147/211/238个token更换专家集合，
顺序变化643/803/920个token。尚未按新路由完成量化/专家/共享
分支，后续必须重建分组与行映射，不能沿用旧行数或旧重排。
...
1. 四分支已到layer1新路由；重建专家分组/行映射，重算量化、
   GU/中间量化/down及共享分支，合并后用自身MLP门值写回。
