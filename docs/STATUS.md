...
最新完成：四分支首层HC残差写回。原41943040项输出逐位恢复，
完整摘要同PLE输入；输入/中间/双项修正改变2118970/5747663/
7747890项BF16，涉及378/1185/1535 token。已到layer1 PLE输入，
尚未重算其依赖新主干的门控/卷积，不是layer1 attention入口。
...
1. 四分支已到PLE输入；重算query norm/门控/gated value/
   norm_conv/因果卷积及残差，先恢复原分支，再接后续层。
