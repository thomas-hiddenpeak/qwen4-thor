...
正在运行 `decode-consistent-paths-20260922`（PID 1930547）：
同一二进制的 1K/4K/8K/44K 分配失败回退→调度重启 HTTP，对照
此前完整矩阵。故障注入库摘要与实际回退日志均校验，未改运行时。
此检查不替换正式输出参考。随后继续核对批量 prefill、视觉/MTP
消费者和正式输出差异。不得伪造 acceptance.json 或用低层测试
覆盖 E2E 未通过项。旧二进制、失败及未启动记录保留。
