...
最新完成：四分支layer1中间量化、down与路由合并。原支完整恢复，
新shape兼容；三个修正支routed F32影响4079/4094/4094个token。
新flat slot与新权重已用于合并；共享分支尚未重算，不能称完整MoE。
本链仅传播layer0干预，layer1仍保留原尺度缺陷。
...
1. 四分支已到layer1 routed F32；用自身mixed重算共享GU/门值/
   SwiGLU/down，合并最终MoE后用自身MLP门值与主干写回。
