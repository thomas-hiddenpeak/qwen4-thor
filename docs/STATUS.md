...
最新完成：四分支layer1 GDN、NormGate/out及注意力HC残差写回，
原支全部恢复。输入/中间/双项修正影响4079/4094/4094个主干位置，
写回改变8765901/13067963/15534688个BF16。尚未MLP HC读取与
MoE路由/专家；后续路由必须重算。不能由传播数量判断最终答案质量。
...
1. 四分支已过layer1注意力HC写回；重算MLP HC norm/read/gate，
   再按各自新输入重算MoE路由、量化、专家与共享分支。
