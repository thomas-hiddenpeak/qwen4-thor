...
最新完成：四分支layer1 HC读取链路，原分支七阶段全恢复。
输入/中间/双项修正的注意力mixed输入改变1048618/2599376/
3453125项BF16，影响1284/2295/2884 token；各支新inject gate
亦重算并保存。终点为线性注意力输入，尚未执行layer1注意力。
...
1. 四分支已到layer1线性注意力输入；重算输入投影/短卷积/
   q/k norm/GDN递推，先恢复原支，再接输出、HC写回及MoE。
