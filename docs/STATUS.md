...
- 已接受运行时仍为 **7364767**，固定性能参考仍为 **fcb5925**。
  九个自有未接受实验源码现已归档并恢复到HEAD/7364767相同字节，
  运行时git diff清空；其他任务文档保留。构建已0且零警告，
  新二进制2a5813f2不同旧归档；质量11/11及五档15条原始
  核对通过、输出同正式、服务0。历史三次完整范围无不利分离，
  近期后两次子集仍4项不利，性能恢复诊断尚未闭合。
- 混合候选未接受：独立答案正式14/15、候选13/15，新增1K求和
  错误可由位置补写单独触发；源码合同支持补写，独立框架同题也
  答错，但不能抹除质量回退或证明全模型正确。恢复基线不是修复。
- 独立M2–4投影41a2a02f已拒绝：已测质量未新增错误，完整五档
  输出同正式，但性能不利；相邻36条原始响应与三服务退出均核对，
  4K TTFT/decode及200K decode对前后正式均不利。证据
  small-m-isolated-adjacent-20260923。不继续重复采样寻找通过。
- 全部失败源码、补丁、二进制与HTTP证据保留。下一步预定相邻
  对照复核恢复版本1K/4K/8K，区分历史完整范围与后续子集口径；
  不运行前置低层测试或旧bench。模型及reference目录只读。
- 本轮证据见[独立投影报告](SMALL_M_ISOLATION_2026-09-23.md)与
  [独立答案报告](INDEPENDENT_ANSWER_E2E_2026-09-23.md)。下方按时间
  追加的“已启动/运行中”为历史过程，应以其后终态和本摘要为准。
