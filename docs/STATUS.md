...
最新完成：四分支layer1共享分支、最终MoE与HC残差写回，原支
全部恢复并同layer2入口。三个修正支写回改变21894180/24145945/
25402375个BF16，影响4079/4094/4094 token。完整layer1传播
已完成，layer2及后续尚未计算；不能据此宣称最终答案因果已确认。
...
1. 四分支已到layer2入口；继续自身主干的HC attention读取、
   线性注意力/GDN与MoE，复用算术但重绑新层权重/算法/状态。
