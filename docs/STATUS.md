...
最新完成：四分支完整PLE链路。原分支所有中间结果/末状态恢复，
最终摘要同layer1 HC read输入；输入/中间/双项修正输出分别改变
2155162/5439036/7438628项BF16，影响1285/2297/2884 token，
均在实际因果支持集内。终点为layer1 HC读取入口，尚未执行该层
norm/mix和注意力。详见 [尺度传播](MOE_SCALE_PROPAGATION_2026-09-24.md)。
...
1. 四分支已过PLE；重算layer1 HC read的norm、混合投影/门值
   和注意力输入，先恢复原分支，再接GDN与后续MoE。
