...
主层 gate 现独立置于 workspace 尾部，两个 GRFrame 顺序借用。frame
只释放自有分配；借用区域须活到排队 Write 完成，或放弃帧时排队 Read
完成。完整 HTTP 与所有权/逐位/布局/时间线检查通过，分配各 362→266/步，
新增 64 KiB 固定预算。详见 [报告](../docs/GRFRAME_GATE_2026-09-21.md)。
此边界仍不是通用执行器；下一步描述实际布局与依赖，不同时改变布局。
