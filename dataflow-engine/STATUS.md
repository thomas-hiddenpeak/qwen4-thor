...
更新：2026-09-23。只描述本目录的设计工作，不替代现有 runner 状态。

## 当前入口校正（2026-09-23）

P7普通serve的logits容量已在7364767按消费者行数缩减，见
[容量合同与验收](SERVE_LOGITS_CAPACITY.md)。源码在ChatServer::Start
按max_seq×vocab×2分配共享prefill logits，各serve消费者显式
请求末行；这已不再是下一项待开发工作。下方按阶段保留的旧
“尚未/下一步”属于当时状态，以本段和runner的
[当前状态](../docs/STATUS.md)为准。

混合decode实验与独立M2–4投影未接受，九个实验文件现已撤下，
运行时源码恢复为接受基线。重建版本质量HTTP11/11原始核对
通过，五档性能验证仍进行中；不得将撤回实验写为引擎已完成。
完整D/P/S执行器、完整状态提交与硬件实测流量预算仍未实现。

恢复验证之后，应先处理已记录的标准messages入口合同：当前
server只拼接role与内容，而固定质量题通过客户端预渲染模板后
走裸prompt，不能证明标准聊天入口正确。下一步先冻结同义的
messages与模型原生模板prompt，做真实HTTP对照，再据结果改动；
不先改kernel、重设答案或扩大性能容忍区间。
...
普通文本 chunk 调度仅在末块请求 logits，全部 HTTP 门禁及每
stream 时间线核对通过；44K TTFT 改善约 0.46%，decode 持平。
[消费者与验收报告](PREFILL_HEAD_DEMAND.md) 保留范围及原始证据。
最终块按需计算末行与每序列行选择仍待实现，不将本阶段当作
完整 P7 完成，也未减少 logits 预留容量。

## P7 单序列末行输出头已接受

普通文本 B=1 prefill/长末块显式只计算最后 trunk 行，默认全行
接口和完整 trunk 输出保持。完整 HTTP、并发、生命周期、数值
参考及时间线通过，按 E1 接受；4K/8K TTFT 改善约 1.6%–1.7%。
详见 [P7 报告](PREFILL_HEAD_DEMAND.md)。预留 logits 容量未缩减，
多序列每序列行选择仍待实现，完整 P7 不能据此标记完成。


## P7 批处理每序列末行已接受

普通文本 B>1 prefill 显式选择各序列末行，复用结束后的另一主干
缓冲，输出按请求打包顺序排列。完整单流、实际 B=2 九轮 27 请求、
交错及生命周期 HTTP 全部通过；17 行 logits、主干与八组续算逐位
一致，真实 HTTP 时间线仅增加末行收集并改变 head 行数。
按性能持平接受，不宣称稳定吞吐提升；详见 [P7 报告](PREFILL_HEAD_DEMAND.md)。
后续按消费者行数缩减logits容量已在7364767接受，见
[SERVE_LOGITS_CAPACITY.md](SERVE_LOGITS_CAPACITY.md)；本段仅记录
每序列末行计算阶段，不再作为当前待办。
